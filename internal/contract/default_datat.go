...
func (d *DefaultDataT) Set(objId string, value types.CoreObj) {
	d.mu.Lock()
	defer d.mu.Unlock()
	d.data[objId] = value
}
...
func (d *DefaultDataT) GetAll() map[string]types.CoreObj {
	d.mu.RLock()
	defer d.mu.RUnlock()

	copiedData := make(map[string]types.CoreObj, len(d.data))
...
func (d *DefaultDataT) NewItem(sid, objId string) (types.CoreObj, error) {
	if types.DefaultRegistry == nil {
		panic("DefaultRegistry is not initialized. The registry package must be imported.")
	}
	registry := types.DefaultRegistry.GetCoreObjRegistry()
	...
	newItem := NewDefaultCoreObj(objId, def)
	d.Set(objId, newItem)
	return newItem, nil
}

// Copy returns a deep copy of the container.
func (d *DefaultDataT) Copy() types.DataT {
	d.mu.RLock()
	defer d.mu.RUnlock()

	newData := &DefaultDataT{
		data: make(map[string]types.CoreObj, len(d.data)),
	}
	for k, v := range d.data {
...
func (d *DefaultDataT) DeepCopy() (types.DataT, error) {
	d.mu.RLock()
	defer d.mu.RUnlock()
...
func (d *DefaultDataT) GetByParam(ctx types.NodeCtx, pname string) (types.CoreObj, error) {
...
func (d *DefaultDataT) NewItemByParam(ctx types.NodeCtx, pname string) (types.CoreObj, error) {
...
func getIOConfigMap(ctx types.NodeCtx, mapKey string) (map[string]IOConfig, bool) {
	nodeDef := ctx.SelfDef()
	if nodeDef == nil {
		return nil, false
	}
...
func GetInputs(ctx types.NodeCtx) (map[string]IOConfig, bool) {
	return getIOConfigMap(ctx, "inputs")
}

// GetOutputs extracts the outputs configuration from a node's context.
func GetOutputs(ctx types.NodeCtx) (map[string]IOConfig, bool) {
