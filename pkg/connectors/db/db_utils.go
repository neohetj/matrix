...
func GetDBConnection(nodePool types.NodePool, dsn string) (*sqlx.DB, bool, error) {
	...
	if strings.HasPrefix(dsn, "ref://") {
		...
		if !ok {
			return nil, false, types.NodeNotFound.Wrap(fmt.Errorf("db client node not found by ref: %s", dsn))
		}
		...
		if !ok {
			return nil, false, types.InternalError.Wrap(fmt.Errorf("node %s is not a SharedNode", dsn))
		}

		dbInstance, err := dbClient.GetInstance()
		if err != nil {
			return nil, false, types.InternalError.Wrap(fmt.Errorf("failed to get db instance from %s: %w", dsn, err))
		}
		db, ok := dbInstance.(*sqlx.DB)
		if !ok {
			return nil, false, types.InternalError.Wrap(fmt.Errorf("shared instance from %s is not a *sqlx.DB", dsn))
		}
		...
		if err != nil {
			return nil, false, types.InternalError.Wrap(fmt.Errorf("failed to create temporary db connection: %w", err))
		}
