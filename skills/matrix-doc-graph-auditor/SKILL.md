...
Use this skill when you need to audit Matrix documentation as a graph instead of reading files one by one. The canonical source now lives in the Matrix repo at `/Users/hejiajun/Documents/开发设计/Evolution/Matrix/skills/matrix-doc-graph-auditor`, and `~/.codex/skills/matrix-doc-graph-auditor` should remain only a symlink to that repo directory.
